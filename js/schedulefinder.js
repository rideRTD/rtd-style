...
var autoComplete = new Bloodhound({
    ...
    limit: 20,
    prefetch: {
        url: appServer.replace('http:', '') + 'schedules/ajax/getAjaxRouteAutoComplete.action?clearCache=true',
        cacheKey: 'routesList',
        filter: function(parsedResponse) {
            ...
            if (moment.tz('February 25th 2017 1AM', 'MMM Do YYYY hA', 'America/Denver').isAfter()) {
                parsedResponse.schedules.push({id: 'R Line (Light Rail)', url: staticServer + 'r-line-schedule-north-weekday.shtml'});
            }

            if (moment.tz('February 25th 2017 1AM', 'MMM Do YYYY hA', 'America/Denver').isAfter()) {
                parsedResponse.schedules.push({id: 'H Line Extension (Light Rail)', url: staticServer + 'h-line-schedule-north-weekday.shtml'});
            }

            for (i = 0; i < parsedResponse.schedules.length; i++) {
                parsedResponse.schedules[i].url = parsedResponse.schedules[i].url.replace('appServer/', appServer);
            }
...
$(document).ready(function() {
    ...
    $('#scheduleFinderTools').typeahead({hint:false}, {
        displayKey: 'id',
        ...
        templates: {
            ...
            suggestion: function(data) {
                var label = (data.id.indexOf('Line') === -1 && data.id.indexOf('Ride') === -1 && data.id.indexOf('Flyer') === -1) ? 'Route ' : '';
                return '<a href="' + data.url + '" onclick="ga(\'send\',\'event\',\'Schedule finder\',\'Tools\',\'' + data.id + '\')">' + label + data.id + '</a>';
            },
