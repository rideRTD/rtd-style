;(function(window) {
    ...
    DropMenu.prototype = {
        ...
        _init: function() {
            ...
            _this.$navBtn = $('.' + _this.options.navBtnClass).not($('.' + _this.options.navMenuClass).find('.' + _this.options.navBtnClass));
            _this.$navMenu = $('.' + _this.options.navMenuClass).not($('.' + _this.options.navMenuClass).find('.' + _this.options.navMenuClass));
            ...
            _this.offevents = 'click.dropmenuoff touchstart.dropmenuoff focusin.dropmenuoff';
        ...
        _initEvents: function() {
            ...
            _this.$navBtn.on(_this.events, function(event) {
                ...
                var touchFlag = false;
                if (!touchFlag) {
                    touchFlag = true; setTimeout(function() {
                        touchFlag = false;
                    }, 100);
                    ...
                    if (theMenu.is(':visible')) {
                        ...
                        if (_this.$navMenu.is(':visible')) {
                            ...
                            _this.$doc.on(_this.offevents, function(event) {
                                ...
                                if (!touchFlag && _this.$navMenu.is(':visible') && !_this.$nav.is(event.target) && _this.$nav.has(event.target).length === 0) {
                                    touchFlag = true;setTimeout(function() { touchFlag = false; }, 100);
            ...
            _this.$toolsToggleBtn.on(_this.events, function() {
                ...
                if (!touchFlag) {
                    ...
                    if (_this.$toolsToggleBtn.attr('aria-expanded') === 'true') {
                        ...
                        _this.$toolsToggle
                            .addClass(_this.options.toolsToggleClass + '--is-closed');
                        _this.$toolsToggle.animate({top:'0rem'}, {duration: 300, queue: false});
                        setTimeout(function() {
                            _this.$toolsToggle.animate({paddingTop: '10px'}, {duration: 50, queue: false});
                        }, 250);
                        ...
                        _this.$toolsToggle
                            .removeClass(_this.options.toolsToggleClass + '--is-closed');
                        _this.$toolsToggle.animate({paddingTop: '0px'}, {duration: 80, queue: false});
                        _this.$toolsToggle.animate({top:'4rem'}, {duration: 300, queue: false});
        ...
        _initAria: function() {
            ...
            _this.$navBtn
                ...
                .attr('aria-controls', function() {
                    return $(this).next('.' + _this.options.navMenuClass).attr('id');
                });
        ...
        _open: function(nav, menu) {
            ...
            _this.$page
                .stop(true, true)
                .animate({ paddingTop: _this.options.menuHeight}, _this.options.slideDur);
        },

        _switch: function(nav, menu) {
            var _this = this;
            _this.$navMenu.filter(':visible')
                .stop(true, true)
