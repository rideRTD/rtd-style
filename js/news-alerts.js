;(function($, appServer) {
    ...
    $.getScript(appServer.replace('http:', '') + 'elbert/HomepageAlerts/getHomePageAlerts.cfm', function() {
    ...
    function insertServicealert(content) {
        ...
        for (i = 0; content.ROWCOUNT > i; i++) {
            if (content.DATA.STORM[i] === 1) {
                ...
                if (content.DATA.DISP_TYPE[i] !== 2) {
                    link =
                        '<a class="' + linkClass + '" href="' + url + '" ' + target + '>' +
                            ((content.DATA.ACTION_TEXT[i] !== null) ? content.DATA.ACTION_TEXT[i] : 'Read more') +
                        '</a>';
                }
                ...
                if (alertCount === 1) {
                    ...
                    '<div class="callout__item">' +
                        '<p class="callout__text">' +
                            '<strong>' + content.DATA.ALERT_TITLE[i] + '</strong><br /> ' +
                            content.DATA.ALERT_DESCRIPTION[i] + '<br /> ' +
                            link +
                        '</p>' +
                    '</div>';
                }
            ...
            html =
                '<div role="alert" class="callout callout--emergency callout--expandable">' +
                    '<h2 class="callout__inner callout__inner--list callout__heading">' +
                        '<button role="button" class="js-expander i i--tools-rideralerts">' +
                            '<i class="i i--tools-rideralerts">' +
                                'Service Alerts <span>(' + alertCount + ')</span>' +
                            '</i>' +
                        '</button>' +
                    '</h2>' +
        ...
        $(html).hide().prependTo('body').rtdExpandable().slideDown();
    }
