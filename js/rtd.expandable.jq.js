    ...
    $.fn.rtdExpandable = function() {
        ...
        if ($expandables.length && $expanders.length) {
            ...
            $expanders.each(function(i) {
                ...
                } else if ($expander.parent().next().hasClass('js-expandable')) {
                ...
                var id = $expandable.is('[id]') ? $expandable.attr('id') : 'expandable-' + uid + '-' + i;
            ...
            if (document.location.hash) {
                if ($expandables.filter(window.location.hash).length) {
                    $expanders.filter('[aria-controls="' + window.location.hash.replace('#', '') + '"]').click();
                }
            }
        }
